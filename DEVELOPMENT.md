# Development Guide - Super Specialists
...
- [Current Specialists](#current-specialists)
- [Creating New Specialists](#creating-new-specialists---detailed-guide)
- [Technical Reference](#technical-reference-from-prd)
...
### Naming Convention
All specialists use **lastname firstname** format (e.g., "Gracchus Tiberius") to maintain thematic consistency with Roman history.

### GUID Range
This mod uses **1500050000-1500059999** (assigned range from community GUID registry).
...
```bash
cd /Users/paul/dev/anno-117-super-specialists && zip -r super-specialists-v$(jq -r '.Version' modinfo.json).zip modinfo.json data/ README.md -x "*.git*" "*.vscode*" "*plans/*" "*references/*" "*scripts/*" "*guids.md"
```
...
- **MINOR** (x.1.x): New specialists, new features, backward compatible
- **PATCH** (x.x.1): Bug fixes, balance tweaks, text corrections

**Examples:**
- `1.0.0` → `1.1.0`: Added new specialist (Gracchus Duilius)
- `1.1.0` → `1.1.1`: Fixed incorrect buff value
- `1.1.1` → `2.0.0`: Completely reworked all specialists (breaking)

**Before packaging:**
1. Update `Version` in `modinfo.json`
2. Add entry to "Version History" section below
...
- ❌ `plans/`, `references/`, `scripts/` (internal docs)
- ❌ `guids.md` (development notes)
